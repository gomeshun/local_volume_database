...
export function makeKinematicsRowId(row: PublicKinematicsRow, index: number): string {
  return [row.object_key, row.source_name, row.source_row, row.star_id || index].join(":");
}
...
export function MemberKinematicsTable({
  ...
  const filteredRows = useMemo(() => {
    ...
    return rows.filter((row) => {
      if (kindFilter !== "all" && row.source_kind !== kindFilter) return false;
      if (!normalizedQuery) return true;
      return [row.star_id, row.source_name, row.source_provider, row.source_ref, row.membership_flag]
        .join(" ")
        .toLowerCase()
    ...
    <div>
      <div className="flex flex-wrap items-center justify-between gap-2">
        <Input
          ...
          placeholder="Search stars or sources..."
          aria-label="Search member stars"
        />

        <div className="text-sm text-muted-foreground">Showing {filteredRows.length.toLocaleString()} rows</div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          >
            ...
            <SelectContent align="end">
              ...
              <SelectItem value="proper_motion">Proper motion</SelectItem>
            </SelectContent>
          </Select>
